...
export default function useGetUpcomingEventsItems() {
  ...
  const uploadImageToStorage = (item, imageFile) => {
    ...
    const storageRef = ref(storage, "UpcomingEvents/" + fileName);
  ...
  const checkFileCountInStorage = () => {
    const listRef = ref(storage, "UpcomingEvents");
    let maxxed = false;
    listAll(listRef)
      .then((response) => {
  ...
  const deleteDocFromFirestore = async (itemId) => {
    let deleted = false;
    const itemDoc = doc(db, "UpcomingEvents", itemId);
